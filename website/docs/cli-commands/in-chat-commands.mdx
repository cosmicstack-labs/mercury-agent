...
| `/view` | Toggle progress view (balanced/detailed) — `Ctrl+T` in CLI |
...
| `/bg kill <id>` | Alias for `/bg cancel` |
| `/bg killall` | Cancel all running background tasks |
| `/bg stopall` | Alias for `/bg killall` |
| `/bg clear` | Prune completed background tasks |
| `/progress` | Show last 10 completed agent steps with timing |
