...
import type { ShortTermMemory, LongTermMemory, EpisodicMemory, MemoryEntry } from '../memory/store.js';
...
export function toModelMessages(recentMemory: MemoryEntry[]): any[] {
  ...
  private async handleMessage(msg: ChannelMessage): Promise<void> {
    ...
    try {
      ...
      if (recentMemory.length > 0) {
        messages.push(...toModelMessages(recentMemory));
      }
